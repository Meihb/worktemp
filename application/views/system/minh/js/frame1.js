...
function bindAdminMenu()
{
    ...
    var attrOne1 = $(this).attr('href');
    $('#main').attr('src',attrOne1);
  });
}
...
function ChangeNav(nav) {
  ...
  $("#menu").find(".items_"+nav).show().find("dl dd").find("ul li a").removeClass("thisclass");
    ...
    $("#menu").find(".items_"+nav).each(function(i,k) {
      $(this).find("dl dd").css('display','none');
      $(this).find("dl dd").find('ul li').find('a').removeClass('thisclass');
      if(i==0){
        $(this).find("dl dd").css('display','block');
        $(this).find("dl dd").find('ul li').eq(0).find('a').addClass('thisclass');
        var attrOne =$(this).find("dl dd").find('ul li').eq(0).find('a').attr('href');
        $('#main').attr('src',attrOne);
        current_link = attrOne;
      }
    });
